...
| **Data Submission Process (Internal)** | [`data-submission-workflow.md`](./workflows/data-submission-workflow.md) | Reference: per active data submission | ⚠️ Drafted v1 (2026-06-17): internal/technical process order and the per-step Jira artifact and template mapping; companion to the submitter-facing Active Data Submission SOP (not cross-linked). Predates DO-EPIC: its artifact map still names the Data Submission user story, the legacy 7e to 7i IDs, and CTDC-1664, so read it alongside the DO-EPIC template until it is revised | CTDC-2110 (CMB v6) |
