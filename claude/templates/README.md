...
| **IndexD Registration Task** | [`indexd-registration-task-template.md`](./indexd-registration-task-template.md) | Loading data — upstream artifact creation (IndexD) | ✅ Drafted v4 (2026-06-01) — external CTDS/DCFS handoff for GUID minting; runs in parallel with the paired Data Loading Task (linked with `Relates`, not blocking); carries the `Data-Concierge` label | CTDC-2060 (AHEP0731 Images-Only — canonical, v4 shape); CTDC-1907 referenced for historical context only |
