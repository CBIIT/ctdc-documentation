...
| **Data Model Update Task** | [`data-model-update-template.md`](./data-model-update-template.md) | Modeling data — internally / CTDC-driven | ✅ Drafted v8 (2026-06-02) — all internally-driven model changes, additive through breaking; 6 sections (milestone tracker, not a term inventory, no counts); records in the internal CTDC CDE Request Workbook (owned by the project, no individual owner); 🔍 DM Federal Lead & Subject Matter Expertise Review section (ServiceNow ticket + DHDM Jira Issue) gates `prod`; Promotion Workflow follows the Data Model Contribution SOP (git PR flow, not a loading pipeline); per-environment progress tracked by native Jira status (no signoff table) | CTDC-2068 (Program curation properties — canonical pilot) |
