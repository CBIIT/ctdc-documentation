...
The two modeling templates (7f and 7g) share one identical seven-section shape as of 7f v9 / 7g v5; they differ only by context — which workbook holds the term-level truth, and which surface is the close trigger (the CRDC Submission Portal for study-driven 7g, Prod plus the downstream Data Hub sync for internally-driven 7f).
...
|---|---|---|---|---|
| **Data Loading Task** | [`data-loading-task-template.md`](./data-loading-task-template.md) | Loading data — end-to-end load | ✅ Drafted v7 (2026-06-03) — loads CRDC submissions into the existing schema; 4-section slim shape (🎯 Load Summary · 📦 Submission & Artifacts · 🚦 Loading Workflow · ✅ Testing Signoff); a dedicated Jenkins job per tier (Dev/QA/Stage/Prod), no lower/upper grouping; routes schema work to the modeling templates and IndexD work to 7h | CMB load (CTDC-1753 lineage); AHEP0731 load (CTDC-2063) |
| **IndexD Registration Task** | [`indexd-registration-task-template.md`](./indexd-registration-task-template.md) | Loading data — upstream artifact creation (IndexD) | ✅ Drafted v5 (2026-06-04) — external CTDS/DCFS handoff for GUID minting; runs in parallel with the paired Data Loading Task (linked with `Relates`, not blocking); carries the `Data-Concierge` label; v5 restructured the Submission & Artifacts table to mirror the Data Loading Task (split AWS Account ID + S3 Bucket, Release Package = directory only, Example→Sample GUID) and dropped the Object Files Location + Consent/ACL rows as indexd.tsv-manifest-captured | CTDC-2060 (AHEP0731 Images-Only — canonical, v5 shape); the 11 paired NCTN-NCORP Index tickets (CTDC-2072–2092, even) aligned to v5; CTDC-1907 referenced for historical context only |
| **Data Modeling for Study Submission** | [`data-modeling-study-submission-template.md`](./data-modeling-study-submission-template.md) | Modeling data — study-driven | ✅ Drafted v5 (2026-06-02) — converged with 7f onto one identical 7-section shape (🎯 Modeling Summary · 📚 CDE Request Workbook · 🔍 DM Federal Lead & SME Review · 🧬 Model Change Details · ✅ Branch & Release Signoff · 🧪 Verification Surfaces · 📝 Notes); differs from 7f only by context (study workbook, Submission Portal close trigger); the study's CDE Request Workbook (owned by the study's Data Concierge) is the term-level source of truth; ticket is a milestone tracker — no specifics, no counts | CTDC-2051 (NCI-MATCH Arm Z1D) ↔ parent CTDC-1666 (retrofitted to v5 shape); CTDC-1799 (CIMAC-CIDC) ↔ parent CTDC-1804 (pending retrofit) |
| **Data Model Update Task** | [`data-model-update-template.md`](./data-model-update-template.md) | Modeling data — internally / CTDC-driven | ✅ Drafted v9 (2026-06-02) — all internally-driven model changes, any SemVer level (additive through breaking); converged with 7g onto one identical 7-section shape (🎯 Modeling Summary · 📚 CDE Request Workbook · 🔍 DM Federal Lead & SME Review · 🧬 Model Change Details · ✅ Branch & Release Signoff · 🧪 Verification Surfaces · 📝 Notes); differs from 7g only by context (internal workbook, Prod + Data Hub sync close trigger); milestone tracker — no specifics, no counts; records in the internal CTDC CDE Request Workbook (owned by the project, no individual owner); 🔍 DM Federal Lead & SME Review (ServiceNow ticket + DHDM Jira Issue) gates `prod`; the Promotion Workflow section was dropped (it duplicated the Data Model Contribution SOP) and replaced by the ✅ Branch & Release Signoff milestone audit trail | CTDC-2068 (Program curation properties — canonical pilot) |
