...
- **Title convention:** `Data Indexing: <Study Name vN>` — reuse the exact `<Study Name vN>` token from the parent Data Submission user story title (Section 7j) verbatim. The board title reads "Data Indexing"; the underlying activity is IndexD registration, called out in the task body.
