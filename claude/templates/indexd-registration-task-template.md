...
- **Title convention:** `Data Indexing: <Study Name vN>` — reuse the exact `<Study Name vN>` token from the parent Data Submission user story title (Section 7e) verbatim. The board title reads "Data Indexing"; the underlying activity is IndexD registration, called out in the task body.
...
2. Confirm this work is IndexD registration, not data loading or modeling. If the work is promoting an existing-and-registered submission through Jenkins, use the Data Loading Task template (Section 7h). If the schema is changing, use a modeling template (Section 7j or 7f).
