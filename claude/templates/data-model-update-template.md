...
3. **Duplicating term-level detail into the ticket.** Listing the specific properties, enum values, relationships, or caDSR CDE codes — or counting how many of each are changing — duplicates the workbook and drifts out of sync. Name the workbook (Section 2); keep the per-term inventory and the tallies there. The ticket records milestones, not terms or counts.
...
10. **Skipping the DM Federal Lead & Subject Matter Expertise Review.** New or updated CDEs and permissible values must be requested from caDSR via a ServiceNow ticket, and a DHDM Jira Issue is created by the DH Coordinator and linked to the Submission Request ticket. Both live in Section 3 and gate the Promotion Workflow in Section 5 — they are not optional even for additive internal changes.
...
4. `### 🧬 **Model Change Details**` — Required field. The milestone-level record of the change. Per-property, per-enum, and per-relationship detail — and the counts — live in the workbook (Section 2), not here.
...
5. `### 🚦 **Promotion Workflow**` — The end-to-end steps follow the **Data Model Contribution SOP** ([`SOP_CTDC_Data_Model_Contribution.md`](https://github.com/CBIIT/ctdc-model/blob/prod/SOP_CTDC_Data_Model_Contribution.md)), which is authoritative. The model promotes by git feature branch → PR into `develop` → PR `develop` → `prod` → tag and GitHub Release, with downstream propagation handled by GitHub Actions. **There is no data-loading Jenkins pipeline in this workflow** — a load runs only if existing data must be re-loaded under the new schema, which is a separate Data Loading Task (Section 7e).
   ...
   **`develop` → `prod` — Stage + Prod, tag, release**
   8. Governance gate (per the DM Updates SOP): confirm the DM Federal Lead & Subject Matter Expertise Review is complete — the DHDM Jira Issue has been created by the DH Coordinator and linked to the Submission Request ticket (Section 3) — and that any requested caDSR codes are recorded in the workbook (or that remaining `TBD` placeholders are an accepted, documented risk).
...
| DM Federal Lead & Subject Matter Expertise Review | 🔍 *(unique to data model update task — the caDSR/PV governance review gate)* |
...
- **No counts.** The number of properties, enums, or permissible values being changed is term-level detail that lives in the CDE Workbook, not the ticket. Write "new optional properties," not "four new optional properties." Counts drift as scope shifts and duplicate what the workbook already tracks.
- **caDSR / ServiceNow and the DM Federal Lead & Subject Matter Expertise Review are not optional.** New or updated CDEs/PVs go through a ServiceNow request to caDSR (Section 3). A DHDM Jira Issue is created by the DH Coordinator and linked to the Submission Request ticket (Section 3), gating `prod` promotion (Section 5). This holds even for additive internal changes.
- **No Acceptance Criteria section.** Model updates are operational SOP work; the completion bar is the ticket reaching Closed once the verification steps embedded in the Promotion Workflow (Section 5) pass. AC belongs on user stories that consume the new schema, not on the model change itself.
...
9. As each environment completes, the assignee advances the ticket's Jira status. The DM Federal Lead & Subject Matter Expertise Review is recorded in Section 3 (the ServiceNow ticket and the DHDM Jira Issue) as those artifacts are created.
...
- **MINOR additive change, internally driven** (new optional property, new optional node, added enum value — e.g., CTDC-2068's Program curation properties) → use this template as-is; it's already trimmed for this case. Verification in the Promotion Workflow is light. Section 2 (internal workbook) and the Section 3 caDSR/ServiceNow + DM Federal Lead & SME Review steps still apply if any CDE/PV is added or changed.
...
- **v8 (2026-06-02)** — Brought the template into lockstep with the canonical pilot CTDC-2068. (1) **No counts:** removed the count ("four") from the example, added an explicit "No counts" rule, and extended antipattern 3 — the number of properties/enums/PVs is term-level detail that lives in the CDE Workbook, not the ticket. (2) Promoted the caDSR/ServiceNow coordination out of the CDE Request Workbook section into its own section, **🔍 DM Federal Lead & Subject Matter Expertise Review** (new Section 3), and replaced the **DC formal sign-off** concept with the **DHDM Jira Issue** created by the DH Coordinator and linked to the Submission Request ticket (owners: DM Fed Lead, DH Lead). (3) Dropped the **Out-of-Scope for Ticket** row from the workbook table — the "not duplicated in the Jira ticket" note now lives in the new Section 3. Renumbered to **6 sections** and repointed the Promotion Workflow governance gate at Section 3.
- **v7 (2026-06-02)** — Removed the **Testing Signoff** section. Per-environment progress (Dev/QA/Stage/Prod) is tracked by the ticket's native Jira status, transitions, and assignee — a signoff table in the description duplicated that. The formal DC sign-off was recorded in the Section 2 caDSR table; the close trigger (Prod validated plus downstream sync verified) lives in the Promotion Workflow. Antipattern 8 was flipped from prescribing a signoff table to warning against duplicating Jira's own tracking in the description. Now **5 sections**.
- **v6 (2026-06-02)** — Corrected the **Promotion Workflow**: it had described the data-*loading* promotion ladder (Jenkins lower/upper-tier pipelines, per-environment data loads). Model contribution does not run a loading pipeline — it is a git feature-branch → PR into `develop` → PR `develop` → `prod` → tag and GitHub Release flow, with downstream propagation via GitHub Actions, exactly per [`SOP_CTDC_Data_Model_Contribution.md`](https://github.com/CBIIT/ctdc-model/blob/prod/SOP_CTDC_Data_Model_Contribution.md). Also corrected the loading-pipeline framing that had leaked into the intro blockquote, the "why this template" paragraph, commitment 3, and the pipeline-anatomy bullet. A Jenkins data load runs only if existing data must be re-loaded under the new schema, which is a separate Data Loading Task (7e).
