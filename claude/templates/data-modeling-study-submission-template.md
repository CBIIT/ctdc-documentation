...
- **Title convention:** `Data Modeling: <Study Name vN>` — reuse the exact `<Study Name vN>` token from the parent Data Submission user story title (Section 7j) verbatim; no acronym or version drift. Internal/non-study model updates (7f) instead use `Data Modeling: <change description>`.
