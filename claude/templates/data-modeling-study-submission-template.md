...
4. `### 🪜 **Steps to Completion**` — Numbered list. The path **this ticket** follows from open to closed — a spine, not a procedure. The *how* of each model edit lives in [`SOP_CTDC_Data_Model_Contribution.md`](https://github.com/CBIIT/ctdc-model/blob/prod/SOP_CTDC_Data_Model_Contribution.md); the DM Fed Lead review runs on the DH DataModel Updates Kanban under its own SOP (Section 3). This ticket tracks **our** work to done — not that board's lane movements, which are the DM Fed Lead's to manage.
   ...
   1. **Workbook captures scope** — the terms in scope are recorded in the CDE Request Workbook (Section 2).
   ...
   5. **Close this ticket** — once the Submission Portal and Data Model Navigator show the new model version (Section 5) and the workbook's Status column is reconciled, move it to its Closed state.

5. `### 🧪 **Verification Surfaces**` — Bullet list. Two end-user surfaces reflect every CTDC model version bump. Both update on the same trigger — the `prod` merge and release tag — and both must show the new model content before this ticket closes.
...
9. As work progresses, the data engineer adds GitHub PR links as Jira comments and advances the ticket through its Jira states — those states carry the branch/release progress a description table used to hold. Section 3 records the DHDM Jira Issue (row 1) first, then the ServiceNow ticket (row 2) once it exists; Section 5 (Notes) accumulates operational context as it develops.
10. **Submission Portal verification plus tag-and-release together are the close trigger.** Once the Submission Portal shows the new model version (Section 5, Verification Surfaces) and the workbook's Status column is fully reconciled, move the ticket to its Closed state.
