...
| DO-ZIP · Megazip Creation Task | CTDC-2104 |
...
| 3 | **DO-DBGAP** | dbGaP Validation Task | `claude/templates/dbgap-validation-task-template.md` | Loading data — consent-group gate | ✅ Drafted v1 (2026-07-09) — 4-section shape (Validation Summary · Submission & Artifacts · Validation Workflow · Verification); runs the established `dbgap_validatation_prod` Prefect deployment (`check_consent_group` on) to reconcile a released submission's ACLs against dbGaP; gates IndexD (DO-INDEX) and Data Loading (DO-LOAD) via `Blocks` until the check passes; `Data-Concierge` label, parent epic CTDC-1664 | CTDC-2141 (dbGaP Validation — first ticket of the pattern) |
...
| 6 | **DO-ZIP** | Megazip Creation Task | `claude/templates/megazip-creation-task-template.md` | CTDC-created artifact (megazip) | ✅ Drafted v2 (2026-07-22) — bundles all of a study's object files into one downloadable `.zip`, **self-mints** its GUID (`dg.4DFC/`) and team-authors its `indexd.tsv`, then indexes it through the **standard DCF Google Drive + CRINTAKE handoff (DO-INDEX) like every CTDC file**, and loads it across Dev/QA/Stage/Prod; 5-section shape (Summary · Submission & Artifacts · Workflow · Verification · Testing Signoff); the only data-management template carrying both a GUID spot-check and a per-tier Testing Signoff; created after Dev spot-check testing and once the submission is marked **Complete** (files moved into the bucket) — built before the QA load rather than after all loading. **v2 corrected the v1 error that claimed megazips are indexed internally with no CTDS/DCFS handoff and no CRINTAKE ticket** — the only megazip-specific difference is the self-minted GUID (vs. a Submission-Portal-assigned one); the DCF/CRINTAKE handoff is identical to DO-INDEX. Canonical CTDC-2104's Index steps were already correct and drove the fix | CTDC-2104 (Create, Index, and Load Megazip for AHEP0731 Image Files) |
